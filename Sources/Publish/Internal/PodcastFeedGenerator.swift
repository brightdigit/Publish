...
internal struct PodcastFeedGenerator<Site: Website>
where Site.ItemMetadata: PodcastCompatibleWebsiteItemMetadata {
  internal let sectionID: Site.SectionID
  internal let itemPredicate: Predicate<Item<Site>>?
  internal let config: PodcastFeedConfiguration<Site>
...
extension PodcastFeedGenerator {
  ...
  fileprivate func makeFeed(
    ...
    PodcastFeed(
      ...
      .group(try items.map(makeItemNode))
    )
  }

  fileprivate func makeItemNode(for item: Item<Site>) throws -> Node<PodcastFeed.ChannelContext> {
