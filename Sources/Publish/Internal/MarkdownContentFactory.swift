...
internal struct MarkdownContentFactory<Site: Website> {
  internal let parser: MarkdownParser
  internal let dateParseStrategy: Date.ParseStrategy

  internal func makeContent(fromFile file: File) throws -> Content {
  ...
  internal func makePage(fromFile file: File, at path: Path) throws -> Page {
    let markdown = try parser.parse(file.readAsString())
    let decoder = makeMetadataDecoder(for: markdown)
    let content = try makeContent(fromMarkdown: markdown, file: file, decoder: decoder)
...
extension MarkdownContentFactory {
  fileprivate func makeMetadataDecoder(for markdown: Ink.Markdown) -> MarkdownMetadataDecoder {
  ...
  fileprivate func makeContent(
    fromMarkdown markdown: Ink.Markdown,
    file: File,
  ...
  fileprivate func resolvePublishingDate(
    fromFile file: File,
    decoder: MarkdownMetadataDecoder
  ) throws -> Date {
