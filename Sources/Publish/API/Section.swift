...
public struct Section<Site: Website>: Location, Sendable {
  ...
  /// The section's absolute path within the website.
...
extension Section {
  ...
  /// - Returns: The matching item, or `nil` if none exists.
  public func item(at path: Path) -> Item<Site>? {
  ...
  /// - Parameters:
  ///   - path: The relative path to add an item at.
  ///   - metadata: The item's site-specific metadata.
  ///   - configure: A closure used to configure the new item.
  ...
  /// - Parameters:
  ///   - path: The path of the item to mutate.
  ///   - mutations: Closure containing the mutations to apply.
  /// - throws: An error if the item couldn't be found, or if the mutation failed.
  public mutating func mutateItem(
    at path: Path,
  ...
  /// - Parameters:
  ///   - predicate: Any predicate to filter the items based on.
  ///   - mutations: Closure containing the mutations to apply.
  public mutating func mutateItems(
    ...
    items = try items.map { item in
      ...
      var item = item
      // `itemIndexesByPath` is kept in sync with `items`, so every existing
      // item's path always resolves to a valid index.
      guard let index = itemIndexesByPath[item.relativePath] else {
        preconditionFailure("Missing index for item at \(item.relativePath)")
      }
      try mutateItem(&item, at: index, using: mutations)
      return item
    }
  ...
  public mutating func removeItems(matching predicate: Predicate<Item<Site>> = .any) {
  ...
  public mutating func sortItems(by sorter: (Item<Site>, Item<Site>) throws -> Bool) rethrows {
...
extension Section {
  internal mutating func addItem(_ item: Item<Site>) {
  ...
  internal mutating func replaceItems(with newItems: [Item<Site>]) {
    items = newItems
    rebuildIndexes()
  }
}
...
extension Section {
  fileprivate mutating func mutateItem(
    ...
    do {
      ...
      if oldTags != newTags {
        ...
        for tag in newTags where !oldTags.contains(tag) {
          itemIndexesByTag[tag, default: []].insert(index)
        }
      }
  ...
  fileprivate mutating func updateLastItemModificationDateIfNeeded(to newDate: Date) {
    if let previous = lastItemModificationDate {
      guard newDate > previous else {
        return
      }
    }
  ...
  fileprivate mutating func rebuildIndexes() {
    itemIndexesByPath = [:]
    itemIndexesByTag = [:]
